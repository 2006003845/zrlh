...
public class SocializeHomeSimple extends BaseSinglePaneActivity {
	...
	public static class BannerExampleFragment extends Fragment {
		...
		public View onCreateView(LayoutInflater inflater, ViewGroup container,
			...
			BitmapDrawable bf = (BitmapDrawable) mContext.getResources().getDrawable(
					R.drawable.testimg);
			Bitmap bitmap = bf.getBitmap();
			byte[] bytes = BitmapUtils.bitmap2Bytes( bitmap );
			View root = inflater.inflate(R.layout.umeng_example_socialize_simple, container, false);
			//=============Mock Data========================================
			...
			ActionBarView socializeActionBar = new ActionBarView(mContext, DESCRIPTOR);
			 ...
			 */
			//Raw
			controller.setShareImage(new UMImage(mContext, bytes));

			//配置分享平台，默认全部
//			SocializeConfig socializeConfig = SocialDemoConfig.getSocialConfig();
...
//			controller.setConfig(socializeConfig);//作用于单个des
			...
			root.findViewById(R.id.commment_bt).setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					...
					UMServiceFactory.getUMSocialService(string, RequestType.SOCIAL).openComment(
							mContext,false);
				}
			});

			root.findViewById(R.id.share_bt).setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					String string = desEd.getText().toString();
					if (TextUtils.isEmpty(string)) {
					...
					UMServiceFactory.getUMSocialService(string, RequestType.SOCIAL).openShare(
							getActivity(),false);
				}
			});

			root.findViewById(R.id.user_center_bt).setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					String string = desEd.getText().toString();
					if (TextUtils.isEmpty(string)) {
					...
					UMServiceFactory.getUMSocialService(string, RequestType.SOCIAL).openUserCenter(
							mContext);
				}
			});
...
//			final DirectShareListener directShareListener = new DirectShareListener() {
//				
//				@Override
//				public void onOauthComplete(String usid, SHARE_MEDIA platform) {
////					if (!TextUtils.isEmpty(usid)) {
////	                    // 授权成功（可打开分享页）
////	                    Toast.makeText(mContext, "授权成功【usid:" + usid + "】",Toast.LENGTH_SHORT).show();
////					} else {
////	                    // 授权失败（不可打开分享页）
...
//			};
			final SnsPostListener snsPostListener = new SnsPostListener() {
				...
				public void onComplete(SHARE_MEDIA platform, int eCode,
					...
					Toast.makeText(getActivity(), "分享结束", Toast.LENGTH_SHORT).show();
				}
			};
			root.findViewById(R.id.direct_share_bt).setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					UMServiceFactory.getUMSocialService(string, RequestType.SOCIAL).setShareImage(shareImage);
					UMServiceFactory.getUMSocialService(string, RequestType.SOCIAL).directShare(mContext,
							SHARE_MEDIA.TENCENT, snsPostListener);
				}
			});

			root.findViewById(R.id.dtest).setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
