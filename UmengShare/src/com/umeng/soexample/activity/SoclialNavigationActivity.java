...
public class SoclialNavigationActivity extends NavigationActivity {
		...
		umSocialService.getUserInfo(this, new FetchUserListener() {
			...
			public void onComplete(int status, SocializeUser user) {
				...
				if(mUser != null  && mUser.mLoginAccount != null 
				        && !TextUtils.isEmpty(mUser.mLoginAccount.getAccountIconUrl())){
	...
	protected void fillMenuContent(View menu) {
		LayoutInflater inflater = (LayoutInflater) SoclialNavigationActivity.this.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		...
		content.findViewById(R.id.msg_user_parent).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				umSocialService.openUserCenter(SoclialNavigationActivity.this);
				slidemenu.hide();
			}
		});
		...
		if(ac != null){
			...
			TextView genderTv = (TextView) content.findViewById(R.id.msg_gender);
			Gender gender = ac.getGender();
			String strGender = gender == null ? "未设置性别" : (gender == Gender.FEMALE ? "女" : "男"); 
		...
		Switch facebookSb = (Switch) content.findViewById(R.id.switch_facebook);
		...
		Switch twitterSb = (Switch) content.findViewById(R.id.switch_twitter);
