package com.example.corporate.base;
...
public abstract class BaseActivity extends Activity {
	 ...
	 * @param content
	 */
	public void showMsgDialog(String msg, int dialogLayoutID, Context context,
			String title, String content) {
		View layout = getLayoutInflater().inflate(dialogLayoutID, null);
		AlertDialog dialog = new AlertDialog.Builder(context).create();
		dialog.setCanceledOnTouchOutside(true);
		dialog.show();
		dialog.getWindow().setLayout(480, 800);
		dialog.getWindow().setContentView(layout);
		dialog.setCanceledOnTouchOutside(true);
		if (title != null)
			setDialogTitle(dialog, dialogLayoutID, title);
		if (content != null)
			setDialogContent(dialog, dialogLayoutID, content);
		setPositiveClickListener(dialog, dialogLayoutID);
		setNegativeClickListener(dialog, dialogLayoutID);
	}
	...
	public abstract OnClickListener setPositiveClickListener(
			AlertDialog dialog, int layoutId);
