...
public class DownloadService extends Service {
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 0:
				...
				contentView.setTextViewText(R.id.noti_tv_progress,
						(readSize < 0 ? 0 : readSize) + "b/s   " + msg.arg1
								+ "%");
				// 更新UI
	...
	private void createNotification(int notifyId) {
		...
		case NOTIFY_DOW_ID:
			...
			mNotification = new Notification(icon, "8888888888", when);

			// 放置在"正在运行"栏目中
			mNotification.flags = Notification.FLAG_ONGOING_EVENT;

			RemoteViews contentView = new RemoteViews(
					mContext.getPackageName(), R.layout.noti_progress_layout);
			contentView.setTextViewText(R.id.noti_tv_cont, "正在下载");

			// 指定个性化视图
			mNotification.contentView = contentView;

			Intent intent = new Intent(this, WidgetTextActivity.class);
		...
		case NOTIFY_OK_ID:
			...
			PendingIntent contentInten2 = PendingIntent.getActivity(mContext,
					0, intent, 0);
			mNotification.setLatestEventInfo(mContext, "", "已下载完毕",
					contentInten2);
			stopSelf();// 停掉服务自身
	...
	private void startDownload() {
		...
		try {
			...
			if (Environment.getExternalStorageState().equals(
				...
				downFile = new File(apkDir + "/" + "cus.apk");
			} else {
				// 未挂载sd卡，保存至手机内置存储空间
				String apkPath = mContext.getCacheDir().getAbsolutePath()
						+ "cus.apk";

				Log.d("手机内部存储路径", apkPath);
				// 修改apk权限
