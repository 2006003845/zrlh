 ...
 */
public class MyPagerTitleStrip extends ViewGroup implements ViewPager.Decor {
	...
	private static final int SIDE_ALPHA = 0x44; // single-byte alpha, 0 =
												// invisible, FF = opaque
	...
	public MyPagerTitleStrip(Context context, AttributeSet attrs) {
		...
		addView(mNext2Text = new TextView(context));
		...
		if (textAppearance != 0) {
			...
			mNext2Text.setTextAppearance(context, textAppearance);
		}
		if (a.hasValue(1)) {
			...
			mNext2Text.setTextColor(textColor);
		}
		mPrevText.setTextColor(0x00ffffff);
		mCurrText.setTextColor(0xffffffff);
		mNextText.setTextColor(0x00ffffff);
		mNext2Text.setTextColor(0x00ffffff);
		...
		if (textSize != 0) {
			...
			mNext2Text.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
		}
		a.recycle();
	 ...
	 *            100 ȫ��͸��0 Ϊȫ͸
	 */
	public void updateNextTextColor(int a) {
		int alpha = ((a * (255 - SIDE_ALPHA) / 100) & 0xFF) + SIDE_ALPHA;
		final int defaultColor = mPrevText.getTextColors().getDefaultColor();
		final int transparentColor = (alpha << 24) | (defaultColor & 0xFFFFFF);
		mNextText.setTextColor(transparentColor);
	}
	...
	protected void onAttachedToWindow() {
		...
		Log.v("DEBUG", "onAttachedToWindow");
	...
	protected void onDetachedFromWindow() {
		Log.v("DEBUG", "onDetachedFromWindow");
	...
	void updateText(int currentItem, PagerAdapter adapter) {
		...
		mNext2Text.measure(childWidthSpec, childHeightSpec);
	 ...
	 * Log.v("DEBUG",
	 * "updateTextPositions position ="+position+",positionOffset="
	 * +positionOffset); if (position != mLastKnownCurrentPage) {
	 * updateText(position, mPager.getAdapter()); } else if (positionOffset ==
	 * mLastKnownPositionOffset) { return; }
	 * 
	 * mUpdatingPositions = true;
	 * 
	 * final int prevWidth = mPrevText.getMeasuredWidth(); final int currWidth =
	 * mCurrText.getMeasuredWidth(); final int nextWidth =
	 * mNextText.getMeasuredWidth(); final int halfCurrWidth = currWidth / 2;
	 * 
	 * final int stripWidth = getWidth(); final int paddingLeft =
	 * getPaddingLeft(); final int paddingRight = getPaddingRight(); final int
	 * paddingTop = getPaddingTop(); final int textPaddedLeft = paddingLeft +
	 * halfCurrWidth; final int textPaddedRight = paddingRight + halfCurrWidth;
	 * final int contentWidth = stripWidth - textPaddedLeft - textPaddedRight;
	 * 
	 * float currOffset = positionOffset + 0.5f; if (currOffset > 1.f) {
	 * currOffset -= 1.f; } final int currCenter = stripWidth - textPaddedRight
	 * - (int) (contentWidth * currOffset); final int currLeft = currCenter -
	 * currWidth / 2; final int currRight = currLeft + currWidth;
	 * 
	 * mCurrText.layout(currLeft, paddingTop, currRight, paddingTop +
	 * mCurrText.getMeasuredHeight());
	 * 
	 * final int prevLeft = Math.min(paddingLeft, currLeft - mScaledTextSpacing
	 * - prevWidth); mPrevText.layout(prevLeft, paddingTop, prevLeft +
	 * prevWidth, paddingTop + mPrevText.getMeasuredHeight());
	 * 
	 * final int nextLeft = Math.max(stripWidth - paddingRight - nextWidth,
	 * currRight + mScaledTextSpacing); mNextText.layout(nextLeft, paddingTop,
	 * nextLeft + nextWidth, paddingTop + mNextText.getMeasuredHeight());
	 * 
	 * mLastKnownPositionOffset = positionOffset; mUpdatingPositions = false; }
	 */

	void updateTextPositions(int position, float positionOffset) {
		Log.v("DEBUG", "updateTextPositions position =" + position
		...
		// final int contentWidth = stripWidth - textPaddedLeft -
		// textPaddedRight;
		...
		// final int currCenter = stripWidth - textPaddedRight - (int)
		// (contentWidth * currOffset);
		// final int currLeft = currCenter - currWidth / 2;
		final int currLeft = textPaddedLeft
				- (int) (stripWidth / 2 * positionOffset);
		...
		final int prevLeft = currLeft - (stripWidth >> 1);
		...
		final int nextLeft = currLeft + (stripWidth >> 1);
		mNextText.layout(nextLeft, paddingTop, nextLeft + nextWidth, paddingTop
				+ mNextText.getMeasuredHeight());

		final int next2Left = currLeft + stripWidth;
		mNext2Text.layout(next2Left, paddingTop, next2Left + nextWidth,
				paddingTop + mNextText.getMeasuredHeight());

		updateNextTextColor((int) (100 * positionOffset));
	...
	private class PageListener extends DataSetObserver implements
		 ...
		 *           positionOffset, int positionOffsetPixels) { Log.v("DEBUG",
		 *           "onPageScrolled position="
		 *           +position+",positionOffset="+positionOffset
		 *           +",positionOffsetPixels="+positionOffsetPixels); if
		 *           (positionOffset > 0.5f) { // Consider ourselves to be on
		 *           the next page when we're 50% of the way there. position++;
		 *           } updateTextPositions(position, positionOffset); }
		 */
		public void onPageScrolled(int position, float positionOffset,
				int positionOffsetPixels) {
			Log.v("DEBUG", "onPageScrolled position=" + position
					+ ",positionOffset=" + positionOffset
			...
			// updateTextPositions(position, positionOffset);
			//
			// updateTextPositions(++position, positionOffset+0.5f);
			// }
