...
public class CoverFlow extends Gallery {
	...
	protected int getChildDrawingOrder(int childCount, int i) {
		Log.i("Log5", "childCount" + childCount);
		Log.i("Log5", "i" + i);
		int first = getFirstVisiblePosition();
		int ss = getSelectedItemPosition();
		int index = ss - first;
		Log.i("Log5", "first" + getFirstVisiblePosition());
		if (i < index) {
			return i;
		} else if (i == childCount - 1) {
			return index;
		} else if (i >= index && i < childCount - 1) {
			return childCount - (i - index) - 1;
		}
		return i;
	}
