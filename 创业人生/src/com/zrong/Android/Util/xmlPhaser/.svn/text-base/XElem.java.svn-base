...
public class XElem extends XContent
{
	...
	//Constructors
	////////////////////////////////////////////////////////////////
	public static final String TAG = "XElem";
	/**
	...
	/**
	 * Return <code>ns().prefix()</code> or null if no namespace is defined for this element.
	 */
	...
	/**
	 * Return <code>ns().uri()</code> or null if no namespace is defined for this element.
	 */
	...
	/**
	 * Get the qualified name of the element. If this element has no namespace or is in the default namespace return the local name. Otherwise return "<prefix>:<name>".
	 */
	public final String qname ()
	{
		if(ns == null || ns.prefix == "")
			return name;
		return new StringBuffer(ns.prefix).append(':').append(name).toString();
	}
	...
	/**
	 * Define the a new namespace for this element and add the "xmlns:<prefix>" attribute. Return the XNs instance used to identify the namespace. Note this does not put this XElem in the namespace unless setNs() is called.
	 */
	...
	/**
	 * Define a namespace for this element by adding the "xmlns:<prefix>" attribute. Return the ns instance passed as the parameter. Note this does not put this XElem in the namespace unless setNs() is called.
	 */
	...
	public final int geti (String name)
	{
		...
		{
			Log.e(TAG,"geti-->"+e.toString() + "  Invalid int attr '" + name + "'='" + v + "'" );
	...
	public final long getl (String name)
	{
		...
		{
			Log.e(TAG, "geti-->"+e.toString() + "  Invalid long attr '" + name + "'='" + v + "'");
	...
	//try
	//{
	//return Float.parseFloat(v);//j2me无此API
	//}
	//catch(Exception e)
	//{
	...
	/**
	 * This is used by the public addAttr() APIs and by XParser to add an attribute where ns is just a prefix string since we might not have resolved it yet.
	 */
