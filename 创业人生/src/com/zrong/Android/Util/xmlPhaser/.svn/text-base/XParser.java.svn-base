/*
 * This source code file is public domain http://sourceforge.net/projects/uxparser
 */
package com.zrong.Android.Util.xmlPhaser;
...
public class XParser
{
	...
	/**
	 * Skip parses all the content until reaching the end tag of the specified depth. When this method returns, the next call to <code>next()</code> will return the element or text immediately following the end tag.
	 */
	...
	/**
	 * Get the current element if <code>type()</code> is ELEM_START or ELEM_END. If <code>type()</code> is TEXT then this is the parent element of the current character data. After ELEM_END this XElem instance is no longer valid and will be reused for further processing. If depth is zero return null.
	 */
	...
	/**
	 * Get the at the current depth. Depth must be between 0 and <code>depth()</code> inclusively. Calling <code>elem(0)</code> will return the root element and <code>elem(depth()-1)</code> returns the current element. If depth is invalid, return null.
	 */
	...
	/**
	 * If the current type is TEXT return the XText instance used to store the character data. After a call to <code>next()</code> this XText instance is no longer valid and will be reused for further processing. If the current type is not TEXT then return null.
	 */
	...
	/**
	 * Parse a [40] element start production. We are passed the first character after the < (beginning of name).
	 */
	...
	/**
	 * Parse a [41] attribute production. We are passed the first character of the attribute name. Return if the attribute had a namespace prefix.
	 */
	private boolean parseAttr (int c, XElem elem) throws Exception
	{
		...
		else
		{
			...
			else if(prefix.toUpperCase().equals("XML"))
			{
				prefix = null;
				name = "xml:" + name;
			}
	...
	/**
	 * Parse an element or attribute name of the format [<prefix>:]name and store result in prefix and name fields.
	 */
	...
	private boolean parseText (int c) throws Exception
	{
		...
		while(true)
		{
			...
			{
				Log.v(TAG, "parseText-->"+e.toString());
				if(gotText)
					throw e;
				return false;
			}
	...
	/**
	 * Skip [3] Space = ' ' '\n' '\r' '\t' Return true if one or more space chars found.
	 */
	...
	/**
	 * Read the next character from the stream: - handle pushbacks - updates the line and col count - normalizes line breaks - throw EOFException if end of stream reached
	 */
	...
	/**
	 * Read the specified char is the amp (&) then resolve the entity otherwise just return the char. If the character is markup then throw an exception.
	 */
	private int toCharData (int c) throws Exception
	{
		...
		while((c = read()) != ';')
			ebuf.append(c);
		//String entity = ebuf.toString().intern();//ԭ
		//
		//if (entity == "lt") return '<';
		//if (entity == "gt") return '>';
	...
	/**
	 * Map the prefix string to a XNs instance declared in the current element or ancestor element.
	 */
	...
	/*
	 * public static long mem() { return Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory(); } public static void main(String[] args) throws Exception { long t1 = System.currentTimeMillis(); System.out.println("start mem  " + (mem()/1024) + "kb"); XElem e = XParser.make(new File(args[0])).parse(); long t2 = System.currentTimeMillis(); System.out.println("finish mem " +
	 * (mem()/1024) + "kb " + (t2-t1) + "ms"); }
	 */
	...
	{
		for(int i = ' ';i < 128;++i)
			//internCache[i] = new String(new char[] { (char)i }).intern();//ԭ
			internCache[i] = String.valueOf(new char[]
			{
				(char)i
			});
	}
