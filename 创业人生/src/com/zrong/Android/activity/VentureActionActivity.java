...
public class VentureActionActivity extends GameActivity {

	public static VentureActionActivity mContext;

	private ArrayList<HashMap<String, Info>> ventureActionData = new ArrayList<HashMap<String, Info>>();
	private SimpleAdapter ventureActionAdapter;
	private ListView ventureActionListView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ventureActionListView = (ListView) this
				.findViewById(R.id.infolist2_listview);
		new VentureActionInfoTask().execute((Void) null);
	}
	...
	private void initData(InfoList infoList) {
		...
		for (Info info : list) {
			...
			map.put("ventureAction", info);
			ventureActionData.add(map);
		}
	}
	...
	private class VentureActionInfoTask extends
		...
		protected InfoList doInBackground(Void... params) {
			...
			try {
				infoList = server.getInfoList(VentureSchoolData.VENTUREACTION,
						"3", "0");
				// 获取政策消息列表
			} catch (WSError e) {
				e.printStackTrace();
			...
			ventureActionListView
					.setOnItemClickListener(new OnItemClickListener() {
						...
						public void onItemClick(AdapterView<?> parent,
							...
							changeActivity(VentureActionContentActivity.class,
									intent);
						}
					});
			if (ventureActionData == null) {
				return;
			}
			ventureActionAdapter = new SimpleAdapter(mContext,
					ventureActionData, R.layout.infolist_item,
					new String[] { "ventureAction" },
					new int[] { R.id.policyinfo_listview2_tv });
			ventureActionListView.setAdapter(ventureActionAdapter);
