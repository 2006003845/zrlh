...
//	}
	...
	/**
	 * Obtains ARGB pixel data from the specified region of this image 
	 * and stores it in the provided array of integers. 
	 * Each pixel value is stored in 0xAARRGGBB format, 
	 * where the high-order byte contains the alpha channel 
	 * and the remaining bytes contain color components for red, 
	 * green and blue, respectively. 
	 * @param rgb data[] an array of integers in which the ARGB pixel data is stored.
	...
	public static InputStream getResourceAsStream(String name) throws IOException {
		...
		if (name.startsWith("/")) {
			try {
				is = MainActivity.mContext.getAssets().open(name.substring(1));
			} 
			catch (Exception e) {
