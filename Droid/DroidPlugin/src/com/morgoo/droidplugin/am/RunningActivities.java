...
/**
 * 正在运行的Activity列表 Created by zhangyong6 on 2015/10/20.
 */
public class RunningActivities {
  ...
  private static Map<Integer, RunningActivityRecord> mRunningSingleStandardActivityList =
      new HashMap<>();
  private static Map<Integer, RunningActivityRecord> mRunningSingleTopActivityList =
      new HashMap<>();
  private static Map<Integer, RunningActivityRecord> mRunningSingleTaskActivityList =
      new HashMap<>();
  private static Map<Integer, RunningActivityRecord> mRunningSingleInstanceActivityList =
      new HashMap<>();

  public static void onActivtyOnNewIntent(Activity activity, ActivityInfo targetInfo,
      ActivityInfo stubInfo, Intent intent) {
    // TODO
  }
  ...
  private static class RunningActivityRecord {
    ...
    private RunningActivityRecord(Activity activity, ActivityInfo targetActivityInfo,
        ActivityInfo stubActivityInfo, int index) {
  ...
  public static void onActivtyCreate(Activity activity, ActivityInfo targetActivityInfo,
      ActivityInfo stubActivityInfo) {
    synchronized (mRunningActivityList) {
      RunningActivityRecord value =
          new RunningActivityRecord(activity, targetActivityInfo, stubActivityInfo,
              findMaxIndex() + 1);
  ...
  private static final Comparator<RunningActivityRecord> sRunningActivityRecordComparator =
      new Comparator<RunningActivityRecord>() {
        @Override
  ...
  private static void doFinshIt(Map<Integer, RunningActivityRecord> runningActivityList) {
    if ((runningActivityList != null)
        && (runningActivityList.size() >= (PluginManager.STUB_NO_ACTIVITY_MAX_NUM - 1))) {
      ...
      if ((record.activity != null) && !record.activity.isFinishing()) {
        ...
        // }
        // Log.d(TAG, "ZYActivity.finish stub=%s target=%s", record.stubActivityInfo,
        // record.targetActivityInfo);
      }
    }
