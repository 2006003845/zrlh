...
public class PluginProcessManager {
  ...
  private static Map<String, ClassLoader> sPluginClassLoaderCache =
      new WeakHashMap<String, ClassLoader>(1);
  ...
  public static ClassLoader getPluginClassLoader(String pkg) throws IllegalAccessException,
  ...
  public static void preLoadApk(Context hostContext, ComponentInfo pluginInfo) throws IOException,
    ...
    synchronized (sPluginLoadedApkCache) {
      ...
      if (object != null) {
        ...
        Object containsKeyObj =
            MethodUtils.invokeMethod(mPackagesObj, "containsKey", pluginInfo.packageName);
        if ((containsKeyObj instanceof Boolean) && !(Boolean) containsKeyObj) {
          final Object loadedApk;
          if (VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB) {
            loadedApk =
                MethodUtils.invokeMethod(object, "getPackageInfoNoCheck",
          ...
          String optimizedDirectory =
              PluginDirHelper.getPluginDalvikCacheDir(hostContext, pluginInfo.packageName);
          String libraryPath =
              PluginDirHelper.getPluginNativeLibraryDir(hostContext, pluginInfo.packageName);
          ...
          if (apk != null) {
            ClassLoader classloader =
                new PluginClassLoader(apk, optimizedDirectory, libraryPath,
  ...
  public static void registerStaticReceiver(Context context, ApplicationInfo pluginApplicationInfo,
      ClassLoader cl) throws Exception {
    List<ActivityInfo> infos =
        PluginManager.getInstance().getReceivers(pluginApplicationInfo.packageName, 0);
          ...
          try {
            ...
            for (IntentFilter filter : filters) {
              BroadcastReceiver receiver =
                  (BroadcastReceiver) cl.loadClass(info.name).newInstance();
  ...
  private static HashMap<String, Application> sApplicationsCache =
      new HashMap<String, Application>(2);

  public static Application getPluginContext(String packageName) throws IllegalAccessException,
  ...
  private static void fakeSystemServiceInner(Context hostContext, Context targetContext) {
    try {
      ...
      try {
        SYSTEM_SERVICE_MAP =
            FieldUtils.readStaticField(baseContext.getClass(), "SYSTEM_SERVICE_MAP");
      } catch (Exception e) {
      ...
      if (SYSTEM_SERVICE_MAP == null) {
        try {
          SYSTEM_SERVICE_MAP =
              FieldUtils.readStaticField(Class.forName("android.app.SystemServiceRegistry"),
                  "SYSTEM_SERVICE_FETCHERS");
        } catch (Exception e) {
          Log.e(
              TAG,
              "readStaticField(SYSTEM_SERVICE_FETCHERS) from android.app.SystemServiceRegistry fail",
              e);
        }
      }
      ...
      if ((SYSTEM_SERVICE_MAP != null) && (SYSTEM_SERVICE_MAP instanceof Map)) {
        ...
        for (Object key : sSYSTEM_SERVICE_MAP.keySet()) {
          ...
          try {
            Method getService =
                serviceFetcher.getClass().getMethod("getService", baseContext.getClass());
  ...
  public static void fakeSystemService(Context hostContext, Context targetContext) {
    if ((VERSION.SDK_INT >= VERSION_CODES.ICE_CREAM_SANDWICH_MR1)
      ...
      Log.i(TAG, "Fake SystemService for originContext=%s context=%s,cost %s ms",
          targetContext.getPackageName(), targetContext.getPackageName(),
